...
32. ~~Evidence-based hardening pass (DESIGN.md v2.0)~~ ✅ — read all 15 verified Capterra reviews of Stone Profit Systems in full (not the rating aggregate) plus SlabWise's 38-question buyer checklist, extracted verbatim recurring complaints, then code-audited BluePlanet against each one before deciding what to fix. **Explicit decision: no reskin** — zero of 15 reviews complained about color/theme, every real complaint was functional; recorded in DESIGN.md §0 with the evidence so it isn't reopened without cause. Landed as: **(0)** DESIGN.md v2.0 — legibility floor, overflow-safe layout contract, one export pattern, Destructive Action Guardrail, each citing the exact complaint it answers. **(1)** Fixed a real latent risk — `InventoryTableClient`'s `selectedIds` was never pruned when filters narrowed, so a bulk hold/write-off could silently include now-hidden rows; now prunes with a toast. Unified 3 different ad-hoc JSON-download idioms (Analytics, Purchasing×2) onto one `src/lib/export.ts` helper. **(2)** Two confirmed real feature gaps closed: a User Admin UI (`/admin/users` — accounts previously could *only* be created by editing the database directly) and Sales Order deposits/AR (`SOPayment` append-only ledger, `balanceDue` always derived, never stored — structurally can't reproduce the SPS review's "deposits show a negative A/R" bug). **(3)** Fabrication line-item depth on quoting (edge profile + cutout upcharges on `SOLineItem`, live-computed total in `QuoteModal`) — "quote versioning" was dropped after investigation showed the premise was wrong (BluePlanet's "quotes" are already-bound orders, not a revisable pre-order entity; flagged as a real architecture finding instead of forcing in a mismatched feature). All four phases verified live end-to-end (user creation + login as the new account, deposit recording with overpayment-guard rejection, fabrication-priced quote creation checked against Postgres).
